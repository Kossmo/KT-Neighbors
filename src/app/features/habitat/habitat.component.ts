  ...
  private async render(allSpecies: Species[]): Promise<void> {
    ...
    const linkSel = linkGroup.selectAll<SVGLineElement, GraphLink>('line')
      ...
      .attr('stroke', d => d.rank === 'genus'
        ? 'rgba(160,90,20,0.75)'
        : 'rgba(120,90,60,0.45)')
      .attr('stroke-width', d => d.rank === 'genus' ? 2.5 : 1.5);
    ...
    const sim = d3.forceSimulation<GraphNode>(nodes)
      ...
      .force('charge',  d3.forceManyBody<GraphNode>().strength(d => -Math.pow(d.r, 1.8) * 3.5))
      .force('collide', d3.forceCollide<GraphNode>().radius(d => d.r + 6).strength(0.9))
